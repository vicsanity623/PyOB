...
class CascadeQueueHandler:
    ...
    def handle_add_to_cascade_queue(self, item: str) -> bytes:
        if not hasattr(self.controller, "add_to_cascade_queue"):
            return json.dumps(
                {
    ...
    def handle_remove_from_cascade_queue(self, item_id: str) -> bytes:
        if not hasattr(self.controller, "remove_cascade_queue_item"):
            return json.dumps(
                {
    ...
    def handle_move_cascade_queue_item(self, item_id: str, direction: str) -> bytes:
        if direction not in ("up", "down"):
            return json.dumps(
                {"error": f"Invalid direction '{direction}'. Must be 'up' or 'down'."}
            ).encode()
        if not hasattr(self.controller, "move_cascade_queue_item"):
            return json.dumps(
                {
